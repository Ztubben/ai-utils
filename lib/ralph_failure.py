...
def reset_on_block_plan(story, reason, prd=None,
    ...
    Pure: computes commands, runs nothing.

    Every story now works on its own branch (PRD #69), so a blocked story's
    work is quarantined from its siblings by construction: the Feature branch
    never saw it, there is nothing to rewind, and the rescue branch that used
    to carry the failed attempt has no job the story branch does not already
    do. What is left is the part a branch cannot do -- pushing the work so a
    human can reach it, saying what failed, and moving the story to
    state:blocked.

    An Orphan Story keeps today's behaviour: the plan is empty but ok, and the
    caller's `attempt_plan` demotion stands unchanged.
    """
    ...
    body = ("%s\n\nReset-on-block: demoting #%s to state:blocked.\n\n"
            "Reason: %s\n\n"
            "The story's work is on its own branch `%s`, which is where it has "
            "been all along -- its Feature's branch and its sibling stories are "
            "untouched."
            % (ATTEMPT_MARKER, number, reason_txt, branch))
