...
One deployment shape closes those controls: GitHub offers neither Approve nor Request changes to the *author* of a pull request, and where Ralph has no identity of its own it opens every pull request as the operator's own account — so the only human in the loop cannot use them, and a Control Plane hold waits forever. There, and only there, an **Approval Marker** stands in: `/approve` or `/request-changes` alone on a line of a pull-request comment, honoured solely from the configured `notify.github` handle, carries exactly the authority the native control would have and is recorded the same way. It is not a command language and adds no authority — it restores one control to the one person who already had it. Giving Ralph its own identity removes the need for it, which is the better fix wherever it is available.
_Avoid_: manual override, human approval command (use "Human Arbitration"); approval bot (use "Approval Marker")
